...
unsigned int levenshtein(const char *word1, const char *word2) {
	unsigned int len1 = (unsigned int)strlen(word1),
		len2 = (unsigned int)strlen(word2);
...
unsigned int fuzzy_strcmp(const char *word1, const char *word2) {
	unsigned int max_len, result;
	max_len = (unsigned int)strlen(word1);
	result = (unsigned int)strlen(word2);
	max_len = MAX(max_len, result);
	result = max_len - levenshtein(word1, word2);
	result *= 100;
	result /= max_len;
